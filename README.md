...
* `#define [标识符] [可计算为最终数字结果的表达式]`。此时该预处理指令将会失去替换文本的功能，而是一个普通的声明语句，但是后续的类型名和所有的变量名不能用`[标识符]`，这个算是对`#define`语义的一种表示。因为这种形式太常见，并通常用于常量声明，于是就加上这个特例了。
* 只要结构体和联合体已经被解析完毕并加入了，后续设置的内存对齐参数和指针类型并不会触发对之前的解析的内容重新内存对齐。
* 对于单次解析的所有符号数量（常量、别名、结构体、联合体、枚举）不能够超出 9223372036854775807 个（包含内部自带基础类型目前是 68 个）。在满足这个 **符号总数量要求** 的前提下，结构体的数量不能够超出 2147483647 个，联合体的数量不能够超过 2147483647 个，别名不能够超出 2147483647 个，枚举不能够超出 2147483647 个，枚举成员数和`#define`常量总量不得超过 9223372036854775807 个。
* 对于所有的数字，内部都是以 64 位的有符号或者无符号整数存储，所以对于`sizeof(ENUM)`，它的大小永远是8个字节。
* 变量类型不必须先声明再使用，该库会自动处理，只需要后续能够补充完整声明即可。
* 对于使用结构体、枚举的时候，`struct`和`enum`关键字可以省略，像 C++ 一样，这是因为所有的符号是不能重名保证实现的特性。
* 对于结构体和联合体的成员标识符可以用`?`表示匿名，通常用于对齐或者保留成员。
